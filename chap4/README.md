...
OSにはユーザーがマウスをクリックするまで待つなど、いつ返ってくるか分からない仕事がある

こういった仕事の対処として、システムの一番下のカーネルでは以下3つが採用されている

* OSが何かをする際に、それを待っているプロセスを一時停止させ、準備ができたらプロセスを再開させる(ファイルなどのブロッキング入力)
* OSが何かをする際に、とりあえずすぐ返す(一部のデータと、まだ続きがある旨) (ノンブロッキング入力)
* プロセスが実行中であれば一時停止して、あらかじめ設定しておいたコールバック関数を呼び出す(シグナル)

Go言語ではio.Readerによるブロッキング読み込みとチャネルを使った通知という２パターンが実装されている

### OSからのシグナルをチャネルで受け取る例

シグナルはプロセス間通信の手段で、プロセスを中断させるなどの用途で使用する
...
* 並列処理しても正しくデータを受け渡す同期構造
* 読み込み・書き込み準備ができるまでブロックする機能

3つ目の機能で、通知のインタフェースとして利用できる
