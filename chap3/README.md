...
- [3章のメモ](#3章のメモ)
  - [io.Reader](#ioreader)
  - [io.Readerの補助関数](#ioreaderの補助関数)
    - [ioutil.ReadAll()](#ioutilreadall)
    - [io.ReadFull()](#ioreadfull)
    - [io.Copy()](#iocopy)
    - [io.CopuBuffer()](#iocopubuffer)
  - [io.Writer,io.Reader以外の入出力インタフェース](#iowriterioreader以外の入出力インタフェース)
  - [入出力の複合インタフェース](#入出力の複合インタフェース)
    - [複合インタフェースのキャスト](#複合インタフェースのキャスト)
  - [よく使われるio.Readerを満たす構造体](#よく使われるioreaderを満たす構造体)
    - [標準入力(os.Stdin)](#標準入力osstdin)
    - [ファイル入力(os.File)](#ファイル入力osfile)
    - [ネットワーク通信の読み込み(net.Conn)](#ネットワーク通信の読み込みnetconn)
    - [メモリに蓄えた内容を読み込むバッファ](#メモリに蓄えた内容を読み込むバッファ)
  - [バイナリ解析関連](#バイナリ解析関連)
    - [必要な部位を切り出す(io.LimiReader / io.SectionReader)](#必要な部位を切り出すiolimireader--iosectionreader)
    - [エンディアン変換](#エンディアン変換)
    - [PNGファイルの分析](#pngファイルの分析)
    - [png画像に秘密のテキストを入れる](#png画像に秘密のテキストを入れる)
  - [テキスト解析](#テキスト解析)
