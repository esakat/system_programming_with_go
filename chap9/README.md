...
パス文字列のクリーンではPOSIX系OSでホームディレクトリを表す`~`(チルダ)の展開はできない  
チルダはOSではなく、シェルが提供する機能なので使う場合は工夫が必要

ホームディレクトリは`os/user`パッケージで取得できる
```Go
my, err := user.Current()
if err != nil {
  panic(err)
...
ディレクトリ内部の要素の情報を集めるのに`os.File`の`Readdir()`or`Readdirnames()`を使った

ディレクトリような木構造を全て辿ることをトラバースと言う

`filepath.Walk()`では深さ優先探索でトラバースできる

Walk()メソッドでは探索の開始地点(root)とトラバース中の各ファイルやディレクトリで呼び出されてほしい関数を渡す

探索して行き着くたびに関数の引数に追加する(コールバック関数)