...
これまでの方法は全て、`必要な情報をまとめて一回で送る`という方式であった  
これでは、全てのデータが用意されるまで送信が遅れる  
チャンクを使うことで、準備ができたデータから送信することができる。特にファイルサイズが大きい場合は有効です

## 速度改善 4. パイプライニング

パイプライニングでは非同期で送受信を行うことで、通信時間を短くする

パイプライニングはサーバ側もブラウザ側も実装が不十分だったため、不遇なものになってしまった

サーバ側の注意点

* サーバー側の状態を変更しない(GETやHEAD)であれば、サーバー側で並列処理を行っていい
* リクエストの順序でレスポンスを返さなければならない

この機能がHTTP/2.0のストリームに繋がっていく
